                    ...
                    <form method="POST" action="{{ route('permission-assignments.assign') }}" class="mb-6">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select id="permission_id" name="permission_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                                    <option value="">Select Permission</option>
                                    @foreach ($permissions as $permission)
                                        <option value="{{ $permission->id }}">{{ $permission->title }}</option>
                                    @endforeach
                                </select>
                                @error('permission_id') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                            </div>
                        </div>
                            ...
                            @foreach ($roles as $role)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @php
                                            $abilities = Bouncer::ability()->whereIn('id', Bouncer::permission()->where('entity_id', $role->id)->where('entity_type', \Silber\Bouncer\Database\Role::class)->pluck('ability_id'))->get();
                                        @endphp
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @foreach ($abilities as $ability)
                                            <form action="{{ route('permission-assignments.revoke') }}" method="POST" class="inline">
                                                ...
                                                <input type="hidden" name="permission_id" value="{{ $ability->id }}">
